...
const handleSearch = () => {
    ...
    searchTerm.timeout = setTimeout(async () => {
        ...
        if(searchTerm.query !== '') {
            const res = await fetch(`http://api.weatherapi.com/v1/search.json?key=234d97e66b0242c7999105100240603&q=${searchTerm.query}`)
...
const getWeather = async (id) => {
  const res = await fetch(`http://api.weatherapi.com/v1/forecast.json?key=234d97e66b0242c7999105100240603&q=id:${id}&days=3&aqi=no&alerts=no`)
  const data = await res.json()
